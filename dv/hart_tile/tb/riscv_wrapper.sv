...
// Execution-core unit-regression wrapper — not the delivery AP hart tile.
// It instantiates the tile-private riscv_core submodule with DV-only instruction
// and local-memory models. Use ap_hart_tile_smoke_tb or SoC DV for integration.
import riscv_pkg::*;
...
module riscv_wrapper #(
  ...
  dcache data_mem_i (
    ...
    .cbo_req_i       (data_cbo_req),
    .cbo_addr_i      (data_cbo_addr[PADDR_W-1:0]),
    .cbo_op_i        (data_cbo_op),
    .cbo_ready_o     (data_cbo_ready),
    .cbo_done_o      (data_cbo_done),
    .cbo_err_o       (data_cbo_err),
