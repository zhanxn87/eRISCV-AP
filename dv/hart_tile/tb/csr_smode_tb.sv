...
module csr_smode_tb;
  ...
  initial begin
    ...
    read_csr(CSR_MENVCFG, MENVCFG_ZICBOM_RESET);
    check(menvcfg_cbie == MENVCFG_CBIE_INVAL && menvcfg_cbcfe,
          "Zicbom menvcfg reset policy is not enabled");
    write_csr(CSR_MENVCFG, '0);
    read_csr(CSR_MENVCFG, '0);
    write_csr(CSR_MENVCFG, xlen_t'(MENVCFG_CBIE_RESERVED) << MENVCFG_CBIE_LSB);
    read_csr(CSR_MENVCFG, '0);
    write_csr(CSR_MENVCFG, MENVCFG_ZICBOM_RESET);
    ...
    // AP exposes supervisor external interrupt on irq[9]; irq[11] is MEIP.
    irq[9] = 1'b1;
    #1;
