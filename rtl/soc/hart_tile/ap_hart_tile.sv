...
// Single AP hart tile structural boundary and the only per-hart integration
// unit instantiated by ap_cluster. The execution pipeline lives in core/;
// memory, Sv39, private L1 caches, and AXI adapters live in mem/.
import ap_soc_pkg::*;
import riscv_pkg::*;
...
module ap_hart_tile #(
  ...
  // Private execution-core to memory-frontend virtual interfaces.
  logic imem_req;
  logic imem_ready;
  logic imem_rvalid;
  ...
  logic data_fence;
  logic data_cbo_req;
  logic data_cbo_ready;
  xlen_t data_cbo_addr;
  cbo_op_e data_cbo_op;
  logic data_cbo_done;
  logic data_cbo_err;
  logic data_cbo_page_fault;
  ...
  riscv_core #(
    ...
    .data_cbo_req_o(data_cbo_req),
    .data_cbo_ready_i(data_cbo_ready),
    .data_cbo_addr_o(data_cbo_addr),
    .data_cbo_op_o(data_cbo_op),
    .data_cbo_done_i(data_cbo_done),
    .data_cbo_err_i(data_cbo_err),
    .data_cbo_page_fault_i(data_cbo_page_fault),
  ...
  ap_hart_memory_frontend #(
    ...
    .data_cbo_req(data_cbo_req),
    .data_cbo_ready(data_cbo_ready),
    .data_cbo_addr(data_cbo_addr),
    .data_cbo_op(data_cbo_op),
    .data_cbo_done(data_cbo_done),
    .data_cbo_err(data_cbo_err),
    .data_cbo_page_fault(data_cbo_page_fault),
