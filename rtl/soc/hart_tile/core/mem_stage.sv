...
module mem_stage #(
  ...
  // Cache-block management transaction. CBO uses store translation and
  // reports store page/access faults without transferring a data word.
  ...
  assign mem_op = ex_mem_i.valid & (ex_mem_i.mem_load | ex_mem_i.mem_store |
                                    ...
                                    (ex_mem_i.cbo_op != CBO_NONE) | ex_mem_i.fence);
  ...
  assign cbo_op = ex_mem_i.cbo_op != CBO_NONE;
  assign fence_response = ex_mem_i.valid && ex_mem_i.fence && data_fence_done_i;
  assign cbo_response = ex_mem_i.valid && cbo_op && data_cbo_done_i && mem_pending_q;
  assign mem_response_valid = ex_mem_i.fence ? fence_response :
                              cbo_op ? cbo_response :
                              (data_resp_valid_i & mem_pending_q);
  assign response_rdata = ex_mem_i.fence || cbo_op ? '0 : data_rdata_i;
  assign response_err = ex_mem_i.fence ? data_fence_err_i :
                      cbo_op ? data_cbo_err_i : data_err_i;
  assign mem_fault_valid = mem_response_valid && !ex_mem_i.fence && response_err;
  assign mem_fault_store = ex_mem_i.mem_store || cbo_op ||
                           ((ex_mem_i.atomic_op != ATOMIC_NONE) &&
                            (ex_mem_i.atomic_op != ATOMIC_LR));
  assign mem_fault_valid_o = mem_fault_valid;
  always_comb begin
    if (data_page_fault_i || data_cbo_page_fault_i)
  ...
  assign data_cbo_req_o = mem_op && cbo_op && !mem_pending_q;
  assign data_cbo_addr_o = ex_mem_i.data_addr;
  assign data_cbo_op_o = ex_mem_i.cbo_op;
  assign data_req_o   = mem_op && !ex_mem_i.fence && !cbo_op && !mem_pending_q;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (((data_req_o && data_req_ready_i) ||
           (data_cbo_req_o && data_cbo_ready_i)) && !data_resp_valid_i &&
          !data_cbo_done_i) begin
        mem_pending_q <= 1'b1;
