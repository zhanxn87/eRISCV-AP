...
module csr_file #(
  ...
  output logic [1:0]      menvcfg_cbie_o,
  output logic            menvcfg_cbcfe_o,
  ...
  function automatic xlen_t sanitize_menvcfg(input xlen_t value);
    xlen_t sanitized;
    begin
      sanitized = value & MENVCFG_ZICBOM_WRITABLE_MASK;
      if (sanitized[MENVCFG_CBIE_LSB +: 2] == MENVCFG_CBIE_RESERVED)
        sanitized[MENVCFG_CBIE_LSB +: 2] = MENVCFG_CBIE_ILLEGAL;
      sanitize_menvcfg = sanitized;
    end
  endfunction
  ...
  always_comb begin
    ...
    if (csr_unimplemented_user_hpm) begin
      ...
      CSR_MENVCFG:       csr_rdata_o = HAS_ZICBOM ? menvcfg_q : '0;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      mcountinhibit_q <= '0;
      mcounteren_q    <= '0;
      menvcfg_q        <= HAS_ZICBOM ? MENVCFG_ZICBOM_RESET : '0;
      mcycle_q       <= '0;
      minstret_q     <= '0;
      mhpmcounter3_q <= '0;
      mhpmcounter4_q <= '0;
      ...
      if (debug_enter_i) begin
        ...
        unique case (csr_addr_i)
          ...
          CSR_MCOUNTEREN:    mcounteren_q <= HAS_MCOUNTEREN ?
                                               ...
                                               '0;
          CSR_MENVCFG:      menvcfg_q <= HAS_ZICBOM ?
                                               sanitize_menvcfg(csr_wvalue) : '0;
  ...
  assign menvcfg_cbie_o   = menvcfg_q[MENVCFG_CBIE_LSB +: 2];
  assign menvcfg_cbcfe_o  = menvcfg_q[MENVCFG_CBCFE_BIT];
