...
package riscv_pkg;
  ...
  localparam bit HAS_ZICBOM       = 1'b1;
  ...
  // Zicbom privilege controls in menvcfg.  S-mode CBO.INVAL can be
  // disabled or downgraded to FLUSH; CLEAN/FLUSH themselves require CBCFE.
  // AP boots with the native operations enabled.
  localparam int unsigned MENVCFG_CBIE_LSB = 4;
  localparam int unsigned MENVCFG_CBCFE_BIT = 6;
  localparam logic [1:0] MENVCFG_CBIE_ILLEGAL = 2'b00;
  localparam logic [1:0] MENVCFG_CBIE_FLUSH   = 2'b01;
  localparam logic [1:0] MENVCFG_CBIE_RESERVED = 2'b10;
  localparam logic [1:0] MENVCFG_CBIE_INVAL   = 2'b11;
  localparam xlen_t MENVCFG_ZICBOM_WRITABLE_MASK =
      (xlen_t'(2'b11) << MENVCFG_CBIE_LSB) |
      (xlen_t'(1) << MENVCFG_CBCFE_BIT);
  localparam xlen_t MENVCFG_ZICBOM_RESET =
      (xlen_t'(MENVCFG_CBIE_INVAL) << MENVCFG_CBIE_LSB) |
      (xlen_t'(1) << MENVCFG_CBCFE_BIT);
  ...
  // Zicbom cache-block management operations.  CBO addresses select one
  // implementation-defined cache block; AP standardizes that block at 64 B.
  typedef enum logic [1:0] {
    CBO_NONE  = 2'd0,
    CBO_INVAL = 2'd1,
    CBO_CLEAN = 2'd2,
