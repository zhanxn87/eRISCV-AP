...
// managers. It is private to ap_hart_tile, whose structural wrapper only
// connects this frontend to the execution-core and external tile interfaces.
import ap_soc_pkg::*;
import riscv_pkg::*;
import sv39_pkg::*;
...
module ap_hart_memory_frontend #(
  ...
  // CBO is translated as a store, then handled locally by the D-Cache.
  input logic data_cbo_req,
  output logic data_cbo_ready,
  input xlen_t data_cbo_addr,
  input cbo_op_e data_cbo_op,
  output logic data_cbo_done,
  output logic data_cbo_err,
  output logic data_cbo_page_fault,
  ...
  logic dcache_cbo_ready, dcache_cbo_done, dcache_cbo_err;
  ...
  assign data_access = (data_we || data_cbo_req) ? SV39_ACCESS_STORE : SV39_ACCESS_LOAD;
  ...
  assign data_req_ready = (d_state_q == D_IDLE) && mmu_d_req_ready && !data_cbo_req;
  assign data_cbo_ready = (d_state_q == D_IDLE) && mmu_d_req_ready && !data_req;
  assign mmu_d_req_valid = (data_req && data_req_ready) ||
                           (data_cbo_req && data_cbo_ready);
      ...
      unique case (d_state_q)
        D_IDLE: begin
          if (mmu_d_req_valid) begin
            ...
            d_cbo_q <= data_cbo_req;
            d_cbo_op_q <= data_cbo_op;
            d_state_q <= D_MMU_WAIT;
          end
        end
        D_MMU_WAIT: begin
        ...
        D_PHYS_REQ: begin
          if (!d_cbo_q || dcache_cbo_ready)
            d_state_q <= D_PHYS_WAIT;
        end
        D_PHYS_WAIT: begin
          if (d_cbo_q ? dcache_cbo_done : dcache_router_resp_valid)
            d_state_q <= D_IDLE;
        end
  ...
  always_comb begin
    ...
    data_cbo_done = 1'b0;
    data_cbo_err = 1'b0;
    data_cbo_page_fault = 1'b0;
    if (d_state_q == D_PHYS_WAIT && d_cbo_q && dcache_cbo_done) begin
      data_cbo_done = 1'b1;
      data_cbo_err = dcache_cbo_err;
    end else if (d_state_q == D_PHYS_WAIT && dcache_router_resp_valid) begin
      data_resp_valid = 1'b1;
      data_rdata = dcache_router_rdata;
      data_err = dcache_router_err;
    end else if (d_state_q == D_FAULT_RESP && d_cbo_q) begin
      data_cbo_done = 1'b1;
      data_cbo_err = 1'b1;
      data_cbo_page_fault = d_page_fault_q;
  ...
  sv39_mmu_ctrl mmu_ctrl_i (
    ...
    .d_req_vaddr_i(data_cbo_req ? data_cbo_addr : data_addr),
    .d_req_satp_i(hart_satp),
    .d_req_privilege_i(data_privilege),
    .d_req_access_i(data_access),
    .d_req_cbo_i(data_cbo_req && data_cbo_ready),
  ...
  dcache_cpu_router dcache_cpu_router_i (
    .clk(clk),
    .rst_n(rst_n),
    .cpu_req_i(d_state_q == D_PHYS_REQ && !d_cbo_q),
  ...
  dcache dcache_i (
    ...
    .cbo_req_i(d_state_q == D_PHYS_REQ && d_cbo_q),
    .cbo_addr_i(d_paddr_q),
    .cbo_op_i(d_cbo_op_q),
    .cbo_ready_o(dcache_cbo_ready),
    .cbo_done_o(dcache_cbo_done),
    .cbo_err_o(dcache_cbo_err),
