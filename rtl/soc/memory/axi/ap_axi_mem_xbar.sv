...
module ap_axi_mem_xbar
  import ap_soc_pkg::*;
(
  input logic clk_i,
  input logic rst_ni,

  AXI_BUS.Slave  slv_axi_i [AP_AXI_MEM_INGRESS_PORTS-1:0],
  AXI_BUS.Master mst_axi_o [AP_AXI_MEM_EGRESS_PORTS-1:0]
);
  ...
  localparam axi_pkg::xbar_cfg_t XBAR_CFG = '{
    NoSlvPorts: AP_AXI_MEM_INGRESS_PORTS,
    NoMstPorts: AP_AXI_MEM_EGRESS_PORTS,
    MaxMstTrans: AP_AXI_MEM_MAX_OUTSTANDING,
    MaxSlvTrans: AP_AXI_MEM_MAX_OUTSTANDING,
    FallThrough: 1'b0,
  ...
  localparam xbar_rule_t [XBAR_CFG.NoAddrRules-1:0] XBAR_RULES = '{
    '{idx: AP_AXI_MEM_EGRESS_DDR,
      start_addr: AP_DDR_BASE,
      end_addr: AP_DDR_LIMIT},
    '{idx: AP_AXI_MEM_EGRESS_ERROR,
      start_addr: '0,
      end_addr: AP_DDR_BASE}
  };

  logic [AP_AXI_MEM_INGRESS_PORTS-1:0] xbar_default_enable;
  logic [AP_AXI_MEM_INGRESS_PORTS-1:0][$clog2(AP_AXI_MEM_EGRESS_PORTS)-1:0]
      xbar_default_port;

  AXI_BUS #(
    .AXI_ADDR_WIDTH(AP_PADDR_W),
    .AXI_DATA_WIDTH(AP_AXI_DATA_W),
    .AXI_ID_WIDTH(AP_AXI_MEM_MST_ID_W),
    .AXI_USER_WIDTH(AP_AXI_USER_W)
  ) xbar_mst_axi [AP_AXI_MEM_EGRESS_PORTS-1:0] ();
  ...
  for (genvar port = 0; port < AP_AXI_MEM_EGRESS_PORTS; port++) begin : gen_id_width
    axi_iw_converter_intf #(
      .AXI_SLV_PORT_ID_WIDTH(AP_AXI_MEM_MST_ID_W),
      .AXI_MST_PORT_ID_WIDTH(AP_AXI_SLV_ID_W),
      .AXI_SLV_PORT_MAX_UNIQ_IDS(AP_AXI_MEM_MAX_OUTSTANDING),
      .AXI_SLV_PORT_MAX_TXNS_PER_ID(1),
      .AXI_SLV_PORT_MAX_TXNS(AP_AXI_MEM_MAX_OUTSTANDING),
      .AXI_MST_PORT_MAX_UNIQ_IDS(AP_AXI_MEM_MAX_OUTSTANDING),
