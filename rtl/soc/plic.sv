...
// AP PLIC: 32 global interrupt sources, one hart, and architecturally distinct
...
module plic #(
  ...
  assign enable_write_data = merge_bytes(
      source_word(enable_q[enable_context], int'(enable_word)), wdata_i, be_i);
  ...
  always_comb begin
    ...
    if (req_i && we_i && threshold_access) begin
      threshold_write_data = merge_bytes(
          {{(32-PRIORITY_BITS){1'b0}}, threshold_q[context_index]}, wdata_i, be_i);
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      resp_valid_o <= req_i && in_window && !we_i;
      ...
      if (req_i && in_window && !we_i) begin
        if (priority_access)
          ...
          rdata_o <= source_word(enable_q[enable_context], int'(enable_word));
        else if (threshold_access)
          rdata_o <= {{(32-PRIORITY_BITS){1'b0}}, threshold_q[context_index]};
        else if (claim_access)
          rdata_o <= {{(32-ID_WIDTH){1'b0}}, highest_id[context_index]};
        else
          rdata_o <= '0;
      end
    end
