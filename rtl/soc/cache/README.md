...
The current synthesizable AP L1 cache controllers are in this transition
location:
...
Their final ownership is `ap_hart_tile`: ITLB precedes I-Cache, DTLB precedes
D-Cache or the uncached peripheral path, and the shared PTW uses a dedicated
cacheable D-Cache port. `rtl/soc/cache/` may move beneath that tile during the
subsystem extraction; cache behavior must not move into the shared AXI fabric.
